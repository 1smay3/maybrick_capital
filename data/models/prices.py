...
class PricesDataHandler:
    def __init__(self, data_gatherer, data_store, interval, sub_directory):
        ...
        self.sub_directory = (
            sub_directory  # Define the subdirectory for storing price data
        )
    ...
    async def gather_and_store_data(self):
        ...
        await self.data_gatherer._fetch_all_data(
            self.build_url, self._process_data, self.sub_directory
        )
    ...
    def _build_adj_close_frame(self, key):
        ...
        self.data_store.write_parquet(
            sorted_df, "processed/market_data", "prices.parquet"
        )
    ...
    def _generate_total_returns(self):
        ...
        total_returns = pct_change(
            processed_prices, lookback=1
        )  # Calculate pct_change over 1 period
        # Save total returns
        self.data_store.write_parquet(
            total_returns, "processed/market_data", "total_return.parquet"
        )
    ...
    def build_base_frame(self, start_date=DATA_START_DATE):
        ...
        # Load total returns
        total_returns = self.data_store.read_parquet(
            "processed/market_data", "total_return.parquet"
        )
